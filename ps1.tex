...
\begin{solution}   \\
\\
- factorial grows faster than the constant
$$~~ f(n) = n! + 34, ~~ g(n) = n!; ~~ n! + 34 = \Theta(n!);$$
\\
- cubic grows faster than the quadratic
$$~~ f(n) = n^3 + n^2, ~~ g(n) = n^3; ~~ n^3 + n^2 = \Theta(n^3)$$
\\
- exponential grows faster than the polynomial
$$~~ f(n) = 4^n + n^{3.5}, ~~g(n) = ; ~~ 4^n + n^{3.5} = \Theta(4^n)$$
\\
- polynomial to the power of 3.5 grows faster than the polynomial into the power of 1
$$~~ f(n) = n^{3.5} - 5n, ~~g(n) = n^{3.5}; ~~ n^{3.5} - 5n = \Theta(n^{3.5}) $$
\\
- there is only one term, so the term itself is the tight bound
$$~~f(n) = 5^{n}, ~~g(n) = 5^{n}, ~~5^{n} = \Theta(5^{n})$$
\\
- constants do not matter and $n^2\log(n)$ grows faster than a quadratic function
$$~~f(n) = n^2\log(n) + 100n^2, ~~g(n) = n^2\log(n); ~~n^2\log(n) + 100n^2 = \Theta(n^2\log(n))$$
\\
- logarithmic function grows faster with comparison to the constant
$$~~f(n) = \log(n) + 23, ~~g(n) = \log(n); ~~\log(n) + 23 = \Theta(\log(n))$$
\\
- $n^n$ is growing faster than the $\log(n)$
...
- $f(n) = 4^{\log_2 n} = 2^{2{\log_2 n}} = 2^{\log_2 n^{2}} = n^2$\\
- $g(n) = n^2 +  n (\log_4 n)^{10};$ or simply $O(n^2)$\\
\\
