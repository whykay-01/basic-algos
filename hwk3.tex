...
def three_way_partition(A, pivot, lo, hi):
    l ← lo
    r ← lo
    u ← hi
...
PROB ANY ARRAY WITH A NUMBER OF DUPLICATE VALUES WOULD BE SUFFICIENT FOR THIS.
...
Sorting it by the number of digits into bins, where every single bin would be only the (the number of digits of an element A[k] is O(i))
