...
Find the $k$-th largest element using the algorithm covered during the lecture, called QuickSelect, which allows us to pick a i-th rank number of the array in linear $O(n)$ time. After such element is found, we could partition around that element and FINISH THIS \\
